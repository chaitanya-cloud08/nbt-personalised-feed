...
// (no live source exists for that section), pins the live "hero" article as
// the featured card, then lays out everything else per the city/interest
// slot pattern in lib/feedLayout.ts.
...
export async function buildFeed(user: UserRecord): Promise<FeedSplit> {
  ...
  const remaining = pool.filter((a) => a.id !== featured.id);
  return { featured, rest: layoutRest(remaining, user.city, user.interests) };
}
